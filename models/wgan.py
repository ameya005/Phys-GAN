...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=DIM, ctrl_dim=0):
        ...
        self.dim = dim
        self.ctrl_dim = ctrl_dim
        #self.lin = nn.Linear(CATEGORY*DIM*DIM, 1024)
        #self.lin2 = nn.Linear(self.ctrl_dim, 1024)
        #self.lin3 = nn.Linear(2048, CATEGORY*DIM*DIM)
        ...
        self.ln2 = nn.Linear(8*8*8*self.dim, self.ctrl_dim)
        #print(self.ln2)
    ...
    def forward(self, input, lv):
        ...
        if len(input.size()) == 4:
           # print('in')
            input = input.view(-1, CATEGORY*DIM*DIM)
        #in1 = self.lin(input)
        #in2 = self.lin2(lv)
