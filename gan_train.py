...
# for PC 3209-13
DATA_DIR = '../morph-data/morph_global_64_train_255.h5'
VAL_DIR = '../morph-data/morph_global_64_valid_255.h5'
...
# Flag suggesting the distribution used to sample the input p1 values for projection step:
# 'from_data' : input p1 values are chosen from the the real data
# 'uniform' : input p1 values are chosen from the uniform distribution
# TODO: add more choices such as random normal
DIST_P1 = 'uniform'
...
def proj_loss(fake_data, real_data, DIST_P1):
    """
    Fake data requires to be pushed from tanh range to [0, 1]
    """
    if DIST_P1 == 'from_data':
        return torch.abs(p1_fn(fake_data) - p1_fn(real_data))
    else:
        raise(NotImplementedError)


def dist_proj_loss(fake_data,dist_p1, DIST_P1 ):
    '''
    This function is used when the input p1 values to the generator is coming from a distribution (such as uniform)
    instead of from the real data.
    :param fake_data: images generated by generator when input is vector [noise, p1 values from defined distribution]
    :param dist_p1: vector of size BATCH_SIZE containing p1 values used as the input (p1 values from defined distribution)
    :param DIST_P1: Flag suggesting the underlying distribution used
    :return: projection loss between the input p1 values and generated images' p1 values.
    '''
    if DIST_P1 == 'uniform':
        return torch.abs(p1_fn(fake_data) - dist_p1)
    else:
        raise(NotImplementedError)
...
def train():
    ...
    for iteration in range(START_ITER, END_ITER):
        ...
        print('---train G elapsed time: {0}'.format(end - start))
        ...
        if DIST_P1 == 'uniform':
            dist_p1 = (0.8 - 0.2)*torch.rand(BATCH_SIZE,1) + 0.2
            dist_p1 = dist_p1.to(device)
            ...
            for i in range(PJ_ITERS):
                ...
                noise = gen_rand_noise()
                noise.requires_grad = True
                dist_fake_data = aG(noise, dist_p1)
                pj_cost = dist_proj_loss(dist_fake_data.view(-1, 1, DIM, DIM), dist_p1, DIST_P1)
                pj_cost = pj_cost.mean()
                pj_cost.backward()
                optimizer_pj.step()
        ...
        for i in range(CRITIC_ITERS):
            ...
            with torch.no_grad():
                ...
                real_p1 = p1_fn(real_data)
                #real_p1_v = real_p1
            end = timer(); print('---gen G elapsed time: {0}'.format(end-start))
            start = timer()
            fake_data = aG(noisev, real_p1).detach()
            end = timer(); print('---load real imgs elapsed time: {0}'.format(end-start))
            start = timer()
            ...
            end = timer(); print('---train D elapsed time: {0}'.format(end-start))
        #---------------VISUALIZATION---------------------
